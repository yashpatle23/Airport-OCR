...
- **R1.4** **Empty ≠ absent.** Unextracted collections use
  `NOT_EXTRACTED_NOT_ABSENT` / `BLOCKED_SOURCE_BYTES_REQUIRED`, never an implied
...
- **R6.1** **Fail loud in the core** — validation errors surface in the report;
  don't swallow them.
- **R6.2** **Fail soft at optional edges** — missing API key, network error, or
  absent optional library must degrade gracefully (fall back, log a short note),
...
- **R7.6** Notebooks are generated via a Python builder (valid nbformat JSON),
